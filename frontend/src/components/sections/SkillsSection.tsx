...
const SkillsSection = () => {
  const skillCategories = [
    ...
    },
    {
      ...
      skills: [
        ...
        { name: "Django Rest Framework", icon: <TbBrandDjango />, color: "#68A063", bgColor: "#68A06320" },
    ...
    {
      title: "Programming Language & others",
  ...
  return (
    <section id="skills" className="py-14 px-6 lg:px-8 bg-secondary/60 rounded-md mt-2">
      <style>{`
        .gradient-text {
              ...
              <CardContent className="p-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {category.skills.map((skill, skillIndex) => (
                    <div
                      key={skillIndex}
                      className="skill-card flex flex-col items-center p-4 rounded-xl bg-secondary/50 hover:shadow-lg transition-all duration-300 group cursor-pointer border border-transparent hover:border-opacity-50"
                      onMouseEnter={(e) => {
                        e.currentTarget.style.borderColor = skill.color + '50';
                        e.currentTarget.style.backgroundColor = skill.bgColor;
                        e.currentTarget.style.boxShadow = `0 8px 25px ${skill.color}20`;
                      }}
                      onMouseLeave={(e) => {
                    ...
                    >
                      <div className="text-2xl mb-3 flex items-center justify-center group-hover:scale-110 transition-transform duration-300"
                           style={{ color: skill.color }}>
                        {skill.icon}
                      </div>
                      <span className="text-sm font-medium text-foreground text-center group-hover:font-semibold transition-all duration-300">
                        {skill.name}
                      </span>
                    </div>
                  ))}
